...
def main():
    ...
    parser.add_argument("--model", default="models/detector_generalized.onnx", help="Path to the ONNX model file.")
    ...
    if detected_files:
        ...
        for i, r in enumerate(detected_files):
            if i < 20:
                print(f"  - {os.path.basename(r['file_path'])} (Predicted: {r['stego_type']}, Confidence: {r.get('confidence', 0):.1%})")
