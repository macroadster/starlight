...
class InfoResponse:
    def __init__(self, **kwargs):
        ...
        self.version = kwargs.get('version', '1.0.0')
        self.description = kwargs.get('description', 'API for scanning Bitcoin transactions for steganography')
...
class BitcoinNodeClient:
    ...
    async def get_transaction(self, tx_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "outputs": [
                {
                    "script_pubkey": "OP_RETURN " + "48656c6c6f6d638783cf57adfe4c75c605f6356fbc91338530e9831e9e16",
                    "value": 0
                }
            ]
        }
    ...
    async def extract_images(self, tx_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "index": 0,
                "data": base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e9AABftCcgAAABJRU5ErkJggg=="),
                "format": "png",
                "size_bytes": 67
            }
        ]
...
class SimpleAPI:
    ...
    def scan_transaction(self, request_data):
        ...
        try:
            ...
            tx_data = asyncio.run(bitcoin_client.get_transaction, request_data.transaction_id)
            
            # Extract images from transaction
            images = []
            if request_data.extract_images:
                extracted_images = asyncio.run(bitcoin_client.extract_images, request_data.transaction_id)
            ...
            logger.error(f"Error scanning transaction {request_data.transaction_id}: {e}")
            return TransactionScanResponse(
                transaction_id=request_data.transaction_id,
                block_height=0,
                timestamp=datetime.utcnow().isoformat(),
    ...
    def scan_image(self, image_data, options):
        ...
        try:
            ...
            image_info = {
                ...
                "format": getattr(image_data, 'filename', 'unknown').split('.')[-1].lower() if '.' in getattr(image_data, 'filename', 'unknown') else 'unknown'
            }
            ...
            for item in request_data.items:
                try:
                    if item.type == "transaction":
                        ...
                        item_stego_count = 1 if tx_result.scan_results.get("stego_detected") else 0
                        ...
                        results.append(BatchItemResult(
                            ...
                            total_images=tx_result.scan_results.get("images_found", 0)
                        ))
                    
                    elif item.type == "image":
                        # Scan base64 image
                        image_bytes = base64.b64decode(item.image_data)
                        img_result = self.scan_image(image_bytes, request_data.scan_options)
