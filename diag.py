...
class AdvancedStegoChecker:
    ...
    def check_alpha_channel(self, clean_img, stego_img):
        ...
        try:
            ...
            alpha_diff = np.abs(
                clean_alpha.astype(np.int32) - stego_alpha.astype(np.int32)
            )
            ...
            return {
                ...
                "alpha_percent_changed": float(
                    np.sum(alpha_diff > 0) / alpha_diff.size * 100
                ),
            }
    ...
    def check_palette_indices(self, clean_path, stego_path):
        ...
        try:
            ...
            index_diff = np.abs(
                clean_indices.astype(np.int32) - stego_indices.astype(np.int32)
            )
    ...
    def comprehensive_check(self, clean_path, stego_path):
        ...
        try:
            ...
            if file_diff.get("file_size_diff", 0) != 0:
                ...
                if file_diff.get("stego_has_eoi_data", False) and not file_diff.get(
                    "clean_has_eoi_data", False
                ):
            ...
            try:
                ...
                if clean_array.shape == stego_array.shape:
                    pixel_diff = np.abs(
                        clean_array.astype(np.int32) - stego_array.astype(np.int32)
                    )
                    ...
                    result["details"]["pixels"] = {
                        ...
                        "percent_changed": float(
                            changed_pixels / pixel_diff.size * 100
                        ),
                    }
            ...
            if (
                not result["has_steganography"]
                and file_diff.get("file_size_diff", 0) != 0
            ):
                result["has_steganography"] = True
    ...
    def find_stego_file(
        self, clean_path, stego_dir, clean_files=None, stego_files_list=None
    ):
        ...
        if stego_files_list is None:
            stego_files_dict = {
                f.stem.lower(): f for f in stego_dir.iterdir() if self._is_image_file(f)
            }
        else:
            stego_files_dict = {
                f.stem.lower(): f for f in stego_files_list if self._is_image_file(f)
            }
        ...
        if stego_files_list is None:
            stego_files_iter = [
                f for f in stego_dir.iterdir() if self._is_image_file(f)
            ]
        else:
    ...
    def check_submission(self, submission_dir):
        ...
        for clean_path in tqdm(clean_files, desc=f"  {submission_dir.name}"):
            ...
            stego_path = self.find_stego_file(
                clean_path, stego_dir, clean_files, stego_files
            )
    ...
    def print_summary(self, results):
        ...
        for sub_result in results:
            ...
            print(
                f"  With steganography: {stats['with_steganography']} ({stats['with_steganography']/max(stats['total'],1)*100:.1f}%)"
            )
            print(
                f"  WITHOUT steganography (IDENTICAL): {stats['without_steganography']} ({stats['without_steganography']/max(stats['total'],1)*100:.1f}%)"
            )
        ...
        print(
            f"Valid steganography: {total_with_stego} ({total_with_stego/max(total_pairs,1)*100:.1f}%)"
        )
        print(
            f"IDENTICAL (no stego): {total_without_stego} ({total_without_stego/max(total_pairs,1)*100:.1f}%)"
        )
