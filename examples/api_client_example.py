...
class StarlightAPIClient:
    def __init__(self, base_url: str = "http://localhost:8080", api_key: str = "demo-api-key"):
    ...
    def scan_transaction(self, transaction_id: str, extract_message: bool = False) -> Dict[str, Any]:
    ...
    def scan_image(self, image_path: str, extract_message: bool = False) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print(f"   Processing Time: {result['scan_results']['processing_time_ms']:.2f}ms")
        
        for i, image in enumerate(result['images']):
            print(f"   Image {i}: {image['size_bytes']} bytes, Prediction: {image['scan_result']['prediction']}")
    except Exception as e:
        print(f"   Error: {e}")
    print()
