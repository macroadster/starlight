...
try:
    ...
    extract_alpha = extract_palette = extract_lsb = extract_exif = extract_eoi = lambda *args: ("", 0)
...
class FastStegoScanner:
    def __init__(self, quick_mode=False, max_workers=4, extract_override=False):
        ...
        print(f"[INIT] Fast scanner initialized (quick_mode={quick_mode}, workers={max_workers}, extract_override={extract_override})")
    ...
    def extract_single(self, img_path, result):
        ...
        if not result.get('is_stego', False) or (self.quick_mode and not self.extract_override):
            return result
