        ...
        if exif:
            ...
            if user_comment is not None:
                if isinstance(user_comment, bytes):
                    if user_comment.startswith(b"ASCII\x00\x00\x00"):
                        message = (
                            user_comment[8:].decode("ascii", errors="ignore").strip()
                        )
                    elif user_comment.startswith(b"UNICODE\x00"):
                        message = (
                            user_comment[8:].decode("utf-16", errors="ignore").strip()
                        )
                    else:
                        try:
                            message = user_comment.decode(
                                "utf-8", errors="ignore"
                            ).strip()
                        except:
                            pass
...
def extract_eoi(image_path):
    ...
    if (
        ...
        or payload.startswith(b"Exif")
        or payload.startswith(b"ICC_PROFILE")
