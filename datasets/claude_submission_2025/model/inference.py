...
class StarlightModel:
    ...
    def __init__(self, model_path="model/detector.onnx", task="detect"):
        ...
        try:
            ...
            print(
                f"Warning: method_config.json not found at {config_path}, using defaults"
            )
            self.method_config = {
...
def test_inference():
    ...
    test_dirs = {"clean": "clean", "stego": "stego"}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="model/detector.onnx", help="Path to ONNX model"
    )
    ...
    if args.test:
        ...
        model = StarlightModel(args.model, task="detect")
        images = list(Path(args.batch).glob("*.png")) + list(
            Path(args.batch).glob("*.bmp")
        )
