...
class StegDataset(Dataset):
    ...
    def __init__(self, clean_dir, stego_dir, transform=None):
        ...
        self.clean_images = sorted(
            list(self.clean_dir.glob("*.png")) + list(self.clean_dir.glob("*.bmp"))
        )
        self.stego_images = sorted(
            list(self.stego_dir.glob("*.png")) + list(self.stego_dir.glob("*.bmp"))
        )
        ...
        print(
            f"Loaded {len(self.clean_images)} clean + {len(self.stego_images)} stego = {len(self.samples)} total"
        )
...
class SimplifiedStegNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.preprocessing_alpha = nn.Conv2d(
            1, 32, kernel_size=5, padding=2, bias=False
        )
    ...
    def _init_preprocessing(self):
        ...
        kernel = (
            torch.FloatTensor([[0, -1, 0], [-1, 4, -1], [0, -1, 0]])
            .unsqueeze(0)
            .unsqueeze(0)
        )
...
def train_model(
    clean_dir="clean", stego_dir="stego", epochs=100, batch_size=8, lr=0.001
):
    ...
    for epoch in range(epochs):
        ...
        if patience_counter >= max_patience:
            print(
                f"Early stopping after {epoch+1} epochs (no improvement for {max_patience} epochs)"
            )
            break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train Starlight steganalysis detector"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--clean-dir", type=str, default="clean", help="Clean images directory"
    )
    parser.add_argument(
        "--stego-dir", type=str, default="stego", help="Stego images directory"
    )
