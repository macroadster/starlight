...
class ClaudeStegGenerator:
    ...
    def read_seed_files(self):
        ...
        if not payloads:
            payloads["default"] = (
                b"Project Starlight: Teaching AI common sense through inscribed wisdom."
            )
    ...
    def png_alpha_extract(self, img_path):
        ...
        payload_bytes = bytes(
            [
                int("".join(payload_bits[j : j + 8])[::-1], 2)
    ...
    def generate_dataset(self, num_images=12):
        ...
        verification_results = {"total": 0, "success": 0, "failed": []}
        ...
        for payload_name, payload_data in payloads.items():
            ...
            for i in range(num_images):
                ...
                method_name, file_ext, embed_func, extract_func = methods[
                    i % len(methods)
                ]
                ...
                try:
                    ...
                    if extracted == payload_data:
                        ...
                        print(
                            f"    Expected {len(payload_data)} bytes, got {len(extracted)}"
                        )
                        verification_results["failed"].append(
                            f"{base_filename}.{file_ext}"
                        )
        ...
        success_rate = (
            (verification_results["success"] / verification_results["total"] * 100)
