...
class ExifEoiDetector(nn.Module):
    def __init__(self, dim=32):
        ...
        self.fc = nn.Sequential(
            nn.Linear(3, dim), nn.ReLU(), nn.Linear(dim, dim), nn.ReLU()
        )
...
class UniversalStegoDetector(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, dim=64):
        ...
        self.feature_fusion = nn.Sequential(
            nn.Linear(fusion_dim, 256), nn.ReLU(), nn.Dropout(0.5), nn.Linear(256, 128)
        )
    ...
    def forward_features(self, rgb, alpha, exif, eoi, palette, indices):
        ...
        combined = torch.cat(
            [f_lsb, f_alpha, f_meta, f_palette, f_palette_index], dim=1
        )
...
class PairedStegoDataset(Dataset):
                ...
                for stego_file in stego_folder.glob("*.json"):
                    try:
                        ...
                        if stego_path.exists() and clean_path.exists():
                            self.stego_pairs.append(
                                {"stego": stego_path, "clean": clean_path}
                            )
                    except Exception:
                        continue
    ...
    def preprocess_image(self, img_path):
        ...
        eoi_feat = torch.tensor(
            [1.0 if get_eoi_payload_size(img_path) > 0 else 0.0], dtype=torch.float
        )
        ...
        def proc(pil, crop_tf, tensor_tf):
            if pil.size[0] < 224 or pil.size[1] < 224:
                ...
                pil = TF.pad(
                    pil,
                    [pad_x // 2, pad_y // 2, pad_x - pad_x // 2, pad_y - pad_y // 2],
                )
        ...
        if img.mode == "P":
            ...
            if pal:
                ...
                palette = torch.from_numpy(
                    np.array(pal).reshape(256, 3) / 255.0
                ).float()
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.stego_pairs):
            ...
            try:
                with open(
                    pair["stego"].with_suffix(pair["stego"].suffix + ".json")
                ) as f:
                    meta = json.load(f)
                tech = meta["embedding"]["technique"]
                if tech == "alpha":
                    label2 = ALGO_TO_ID["alpha"]
                elif tech == "palette":
                    label2 = ALGO_TO_ID["palette"]
                elif tech == "lsb.rgb":
                    label2 = ALGO_TO_ID["lsb"]
                elif tech == "exif":
                    label2 = ALGO_TO_ID["exif"]
                elif tech == "raw":
                    label2 = ALGO_TO_ID["eoi"]
                else:
...
def train_model(args):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(val_pbar):
                ...
                # Update progress bar
                val_pbar.set_postfix({"Loss": f"{val_loss/(batch_idx+1):.4f}"})
        ...
        print(
            f"  Val Loss: {val_loss_avg:.4f}, Val Acc: {best_acc:.2f}% (threshold: {best_thresh:.2f})"
        )
        ...
        if not improved:
            print(
                f"  No improvement. Patience: {early_stopper.counter}/{early_stopper.patience}"
            )
    ...
    if model_path.exists():
        ...
        print(
            f"[FINAL] Best validation accuracy: {best_acc:.2f}% (threshold: {best_thresh:.2f})"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--val_subdir", type=str, default="val", help="Validation subdirectory"
    )
    parser.add_argument(
        "--model", type=str, default="model/detector.pth", help="Path to save the model"
    )
