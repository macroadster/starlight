...
class AlphaDetector(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(4, 16, kernel_size=3, stride=1, padding=1), nn.ReLU(),
...
class EoiDetector(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=3, stride=1, padding=1), nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Conv1d(16, 32, kernel_size=3, stride=1, padding=1), nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(32 * 256, 512), nn.ReLU(),
            nn.Linear(512, 1), nn.Sigmoid()
        )
    def forward(self, x):
        x = self.conv_layers(x)
        ...
        if self.task == 'detect':
            ...
            if "clean" in file_path: # No payload in clean images
                ...
                if self.method == 'alpha':
                    ...
                    payload_padded[:len(actual_payload_bytes)] = [b for b in actual_payload_bytes]
                    payload_label = np.expand_dims(payload_padded, 0)

            return torch.tensor(data, dtype=torch.float32), torch.tensor(payload_label, dtype=torch.float32)
