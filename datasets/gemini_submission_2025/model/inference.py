...
class StarlightModel:
    ...
    def _load_method_config(self) -> Dict:
        ...
        if not os.path.exists(config_path):
            ...
            config_path = os.path.join(
                os.path.dirname(self.model_dir), "method_config.json"
            )
            if not os.path.exists(config_path):
    ...
    def preprocess(self, img_path: str, method: str) -> np.ndarray:
        config = self.method_config.get(
            method, self.method_config["lsb"]
        )  # Fallback to lsb config
    ...
    def _postprocess_alpha_extraction(
        self, model_output: np.ndarray, hint_bytes: bytes, terminator_bytes: bytes
    ) -> str:
        ...
        if not terminator_bytes:
            ...
            if terminator_end == -1:
                terminator_end = len(
                    full_extracted_data
                )  # If terminator not found, read till end

        return full_extracted_data[payload_start:terminator_end].decode(
            "utf-8", errors="ignore"
        )

    def _postprocess_eoi_extraction(
        self, model_output: np.ndarray, hint_bytes: bytes, terminator_bytes: bytes
    ) -> str:
    ...
    def predict(self, img_path: str, task: str, method: str = None) -> Dict[str, Any]:
        ...
        if model_path in self.sessions:
            ...
            return {
                "error": f"Model not found for method: {method}, task: {task} at {model_path}"
            }
        ...
        if task == "detect":
            ...
            terminator_bytes = (
                bytes.fromhex(terminator_bytes_hex) if terminator_bytes_hex else b""
            )

            if method == "alpha":
                extracted_payload = self._postprocess_alpha_extraction(
                    outputs[0], hint_bytes, terminator_bytes
                )
            elif method == "eoi":
                extracted_payload = self._postprocess_eoi_extraction(
                    outputs[0], hint_bytes, terminator_bytes
                )
            else:
                extracted_payload = "Extraction not supported for this method."

            return {"method": method, "extracted_payload": extracted_payload}
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Perform steganography detection or extraction."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Task to perform (detect or extract).",
    )
    parser.add_argument(
        "--method",
        type=str,
        help="Steganography method (e.g., alpha, eoi). If not provided, will attempt to detect from filename.",
    )
