...
class DatasetAnalyzer:
    ...
    def analyze_image(
        self, img_path: Path, label: Optional[str] = None
    ) -> ImageAnalysis:
        ...
        try:
            ...
            if label == "alpha":
                if not has_alpha:
                    is_valid = False
                    issues.append(
                        f"INVALID_LABEL: Alpha steganography labeled on {mode} image without alpha channel"
                    )
            ...
            if label == "palette":
                if not has_palette:
                    is_valid = False
                    issues.append(
                        f"INVALID_LABEL: Palette steganography labeled on {mode} image without palette"
                    )
            ...
            if img_format == "JPEG" and label in ["lsb", "alpha"]:
                issues.append(
                    "WARNING: JPEG lossy compression may corrupt LSB/alpha data"
                )
    ...
    def scan_directory(
        self, dir_path: Path, label: Optional[str] = None
    ) -> List[ImageAnalysis]:
    ...
    def analyze_all(self) -> DatasetStats:
        ...
        if not dataset_dirs:
            print(
                "Warning: No dataset directories found matching pattern '*_submission_*'"
            )
                ...
                if method_dirs:
                    ...
                    for method_dir in method_dirs:
                        ...
                        if method in self.STEGO_METHODS:
                            stego_analyses = self.scan_directory(
                                method_dir, label=method
                            )
        ...
        for analysis in self.analyses:
            for issue in analysis.issues:
                if "INVALID_LABEL" in issue:
                    critical["invalid_labels"].append(f"{analysis.path}: {issue}")
    ...
    def print_summary(self):
        ...
        for method, count in sorted(self.stats.method_distribution.items()):
            pct = (
                (count / self.stats.stego_count) * 100
                if self.stats.stego_count > 0
                else 0
            )
