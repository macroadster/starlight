...
class ProductionValidator:
    ...
    def validate_clean_dataset(self, clean_dir, min_images=10000):
        ...
        if format_breakdown:
            ...
            for fmt, stats in sorted(format_breakdown.items()):
                ...
                print(
                    f"  {fmt:8s}: {fp_count:4d} FP / {total:5d} total ({fp_pct:.2f}%)"
                )
        ...
        if method_breakdown:
            ...
            for method, count in sorted(
                method_breakdown.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def validate_stego_dataset(self, stego_dir, min_per_method=100):
        ...
        missed = [
            r for r in results if not r.get("is_stego", False) and "error" not in r
        ]
        ...
        if method_stats:
            ...
            for method, stats in sorted(method_stats.items()):
                ...
                print(
                    f"  {status} {method:12s}: {detected_count:4d}/{total:4d} ({rate:.1f}%)"
                )
    ...
    def validate_performance(self, test_dir, target_images=1000):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i, img in enumerate(test_images):
                shutil.copy(
                    img, os.path.join(tmpdir, f"test_{i}{os.path.splitext(img)[1]}")
                )
    ...
    def generate_report(self, output_path):
        ...
        perf_pass = self.results.get("performance_metrics", {}).get(
            "meets_target_throughput", False
        )
        ...
        if not clean_pass:
            ...
            if fp_rate > 0.5:
                recommendations.append(
                    "CRITICAL: False positive rate too high for production. Review special case logic."
                )
            elif fp_rate > 0.1:
                recommendations.append(
                    "WARNING: False positive rate above target. Consider additional validation."
                )

        if not stego_pass:
            det_rate = self.results.get("stego_validation", {}).get(
                "detection_rate_percent", 0
            )
            recommendations.append(
                f"CRITICAL: Detection rate {det_rate:.1f}% below 95% target. Review method-specific performance."
            )

        if not perf_pass:
            throughput = self.results.get("performance_metrics", {}).get(
                "images_per_second", 0
            )
            recommendations.append(
                f"WARNING: Performance {throughput:.1f} img/s below target. Optimize inference pipeline."
            )

        if overall_pass:
            recommendations.append(
                "SUCCESS: All validation targets met. V4 is production-ready."
            )
...
Expected Directory Structure:
  ...
  └── stego/          # 500+ stego images with JSON sidecars
    ...
    parser.add_argument(
        "--skip-performance", action="store_true", help="Skip performance benchmarking"
    )
    ...
    stego_pass = validator.validate_stego_dataset(
        args.stego, min_per_method=args.min_per_method
    )
