            ...
            with Image.open(img_path) as img:
                ...
                if valid_modes and mode not in valid_modes:
                    ...
                    if method == "alpha" and mode == "RGB":
                        ...
                        suggestions.append(
                            f"Verify label or convert image to compatible mode"
                        )
                ...
                if method == "alpha":
                    ...
                    if not img.mode.endswith("A"):
                        ...
                        suggestions.append(
                            "This is the most critical type of error - must be fixed"
                        )
        ...
        for pattern in patterns:
            for dir_path in glob.glob(pattern):
                ...
                if dir_path.name in self.VALID_MODES:
                    ...
                    print(
                        f"Validated {total} images in {dir_path}, found {invalid} invalid labels"
                    )
                else:
                    ...
                    for method in self.VALID_MODES.keys():
                        ...
                        if method_dir.exists():
                            ...
                            print(
                                f"Validated {total} images in {method_dir}, found {invalid} invalid labels"
                            )
    ...
    def generate_report(self, output_path: Path):
        ...
        with open(output_path, "w") as f:
            ...
            if not self.invalid_labels:
                ...
                f.write(
                    "All steganography method labels are compatible with their image formats.\n"
                )
                return
            ...
            alpha_rgb_count = sum(
                1
                for inv in self.invalid_labels
                if inv.assigned_label == "alpha" and "RGB" in inv.image_mode
            )

            if alpha_rgb_count > 0:
                f.write(
                    f"### 🚨 CRITICAL: {alpha_rgb_count} Alpha Labels on RGB Images\n\n"
                )
                f.write("These are impossible and must be fixed:\n\n")
                f.write("1. **Option A**: Remove the labels entirely (recommended)\n")
                f.write(
                    "2. **Option B**: Verify if images were mislabeled as 'alpha' when they should be 'lsb'\n"
                )
                f.write(
                    "3. **Option C**: Remove images from dataset if they cannot be verified\n\n"
                )
            ...
            if palette_errors > 0:
                f.write(
                    f"### ⚠️  {palette_errors} Palette Labels on True-Color Images\n\n"
                )
                f.write("These should be reviewed:\n\n")
                f.write("1. Verify if these should be labeled as 'lsb' instead\n")
                f.write(
                    "2. Check if palette indices were embedded during conversion\n\n"
                )

            f.write("### Next Steps\n\n")
            f.write(
                "1. Run `scripts/dataset_repair.py` to automatically fix invalid labels\n"
            )
