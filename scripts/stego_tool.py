...
def extract_lsb(image_path, max_bits=1_000_000):
    ...
    if img.mode in ("RGBA", "RGB") or str(image_path).lower().endswith(
        (".png", ".webp", ".jpg", ".jpeg")
    ):
...
def extract_eoi(image_path):
    ...
    if (
        ...
        or payload.startswith(b"<?xpacket")
        or payload.startswith(b"http://ns.adobe.com")
        or payload.startswith(b"GIF89a")
        or payload.startswith(b"\x89PNG")
    ):
        return None, None
    if all(b == 0 for b in payload[: min(20, len(payload))]) or all(
        b == 0xFF for b in payload[: min(20, len(payload))]
    ):
        return None, None
    try:
...
def process_image(file_path, detail=False, diagnose=False):
    ...
    for method, func in extraction_functions.items():
        ...
        if msg:
            conf = (
                _lsb_confidence(msg)
                if method == "lsb"
                else calculate_message_confidence(msg)
            )
    ...
    if detail or diagnose:
        ...
        if status == "STEGO":
            detail_line = (
                f"{file_path}: {status} (method: {best_method}, conf: {conf_str})"
            )
...
# ----------------------------------------------------------------------
def scan_directory(
    directory, detail=False, recursive=False, output=None, diagnose=False
):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_image, fp, detail, diagnose): fp
        ...
        if output.endswith(".csv"):
            with open(output, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["file", "status", "method", "confidence", "message"]
                )
                writer.writeheader()
...
def main():
    parser = argparse.ArgumentParser(
        description="Stego Tool v3: Embed, Extract, Scan (GPU Batched, No SDM)"
    )
    ...
    embed_parser.add_argument(
        "--method", choices=list(embedding_functions.keys()), required=True
    )

    extract_parser = subparsers.add_parser("extract", help="Extract message")
    extract_parser.add_argument("--input", required=True)
    extract_parser.add_argument(
        "--method", default="auto", choices=["auto"] + list(extraction_functions.keys())
    )
    ...
    if args.mode == "embed":
        ...
        if args.method == "exif":
            ...
            save_jpeg_with_eoi_append(
                stego, output_path, stego.info.get("eoi_append", b"")
            )
        else:
        ...
        scan_directory(
            args.directory, args.detail, args.recursive, args.output, args.diagnose
        )
