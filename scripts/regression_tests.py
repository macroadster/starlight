...
class RegressionTestSuite:
    ...
    def test_negatives_resistance(self, model, negative_dataset):
        ...
        if isinstance(negative_dataset, str):
            negative_images = list(Path(negative_dataset).glob("**/*.png"))[:100]  # Sample
        else:
            negative_images = negative_dataset
        
        fp_count = sum(1 for img in negative_images if infer_image(model, str(img)).get('is_steganography', False))
...
def calculate_detection_rates(model, test_data):
    ...
    for method in methods:
        ...
        tp_count = sum(1 for f in stego_files if infer_image(model, f).get('is_steganography', False))
...
def benchmark_inference(model, sample_size=1000, dataset_path=None):
    ...
    if dataset_path:
        ...
        test_images = list(Path("datasets/sample_submission_2025/clean/").glob("*.png"))[:sample_size]
...
def calculate_fpr_by_format(model, test_data, fmt):
    ...
    fp_count = sum(1 for img in clean_images if infer_image(model, img).get('is_steganography', False))
    return (fp_count / len(clean_images)) * 100 if clean_images else 0.0
