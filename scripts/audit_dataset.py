...
def audit_dataset(clean_dir, stego_dir):
    ...
    stego_images = [
        f for f in stego_path_obj.iterdir() if f.suffix.lower() in image_extensions
    ]
    ...
    for stego_filepath in stego_images:
        ...
        try:
            ...
            if not clean_filepath.exists():
                issues["clean_file_not_found"].append(
                    f"{clean_filename} (referenced by {stego_filepath.name})"
                )
                continue
    ...
    for i, (clean_img_path, stego_img_path) in enumerate(pairs_to_check):
        ...
        if clean_img.size != stego_img.size:
            issues["dimension_mismatch"].append(
                f"{stego_img_path.name} (Clean: {clean_img.size}, Stego: {stego_img.size})"
            )

        if clean_img.mode != stego_img.mode:
            issues["mode_mismatch"].append(
                f"{stego_img_path.name} (Clean: {clean_img.mode}, Stego: {stego_img.mode})"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Audit a steganography image dataset using JSON sidecars for pairing."
    )
    parser.add_argument(
        "--clean_dir", required=True, help="Directory containing clean images."
    )
    parser.add_argument(
        "--stego_dir", required=True, help="Directory containing stego images."
    )
