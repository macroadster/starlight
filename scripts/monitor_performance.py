...
class PerformanceMonitor:
    def __init__(self, model_path, dataset_path):
        ...
        self.scanner = StarlightScanner(
            model_path, num_workers=1, quiet=True
        )  # quiet for monitoring
    ...
    def calculate_detection_rates(self, stego_images_by_method):
        ...
        for method, images in stego_images_by_method.items():
            tp_count = sum(
                1
                for img in images
                if self.scanner.scan_file(img).get("is_stego", False)
            )
    ...
    def dataset_quality_metrics(self):
        ...
        entropy = -sum(
            (p / 100) * math.log2(p / 100) for p in balance.values() if p > 0
        )
    ...
    def check_regressions(self, metrics, baseline):
        ...
        if metrics["fpr"] > baseline["fpr"] * 1.1:
            alerts.append(
                f"⚠️  FPR increased: {metrics['fpr']:.2f}% vs {baseline['fpr']:.2f}%"
            )

        # Per-method detection
        for method in baseline.get("detection_rates", {}):
            if (
                method in metrics["detection_rates"]
                and metrics["detection_rates"][method]
                < baseline["detection_rates"][method] * 0.95
            ):
    ...
    def generate_report(self):
        ...
        if stego_dir.exists():
            for method_dir in stego_dir.iterdir():
                if method_dir.is_dir():
                    stego_images_by_method[method_dir.name] = list(
                        method_dir.glob("**/*.png")
                    )
...
def main():
    ...
    parser.add_argument(
        "--update-dashboard", action="store_true", help="Update the dashboard markdown"
    )
