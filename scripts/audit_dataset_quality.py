...
class DatasetQualityAuditor:
    ...
    def scan_dataset(self, dataset_dir):
        ...
        if not stego_dir.exists():
            self.issues["missing_directories"].append(
                {"dataset": dataset_name, "missing": "stego"}
            )
            return

        if not clean_dir.exists():
            self.issues["missing_directories"].append(
                {"dataset": dataset_name, "missing": "clean"}
            )
            return
    ...
    def validate_stego_file(self, dataset_name, stego_file):
        ...
        if not json_file.exists():
            self.issues["missing_json"].append(
                {"dataset": dataset_name, "file": str(stego_file.name)}
            )
        ...
        try:
            ...
            self.issues["invalid_json"].append(
                {"dataset": dataset_name, "file": str(stego_file.name), "error": str(e)}
            )
    ...
    def generate_report(self):
        ...
        for dataset_name, stats in sorted(self.stats.items()):
            ...
            techniques = {
                k.replace("technique_", ""): v
                for k, v in stats.items()
                if k.startswith("technique_")
            }
            if techniques:
        ...
        if "invalid_alpha_label" in self.issues:
            ...
            if count <= 10:
                for issue in self.issues["invalid_alpha_label"]:
                    print(
                        f"      - {issue['dataset']}/{issue['file']} (mode: {issue['mode']})"
                    )
            else:
    ...
    def save_json_report(self):
        ...
        report = {"statistics": dict(self.stats), "issues": dict(self.issues)}
