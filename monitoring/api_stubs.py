...
if FASTAPI_AVAILABLE:
    class InferenceMetrics(BaseModel):
        ...
        prediction: str = Field(..., regex="^(stego|clean)$", description="Model prediction")
    ...
    class Alert(BaseModel):
        ...
        severity: str = Field(..., regex="^(critical|warning|info)$", description="Alert severity")
        status: str = Field(..., regex="^(active|acknowledged|resolved)$", description="Alert status")
        ...
        actions: List[AlertAction] = Field(default_factory=list, description="Alert actions")
    ...
    class AcknowledgeRequest(BaseModel):
        ...
        suppress_until: Optional[datetime] = Field(None, description="Suppress until timestamp")
    ...
    class ModelVersion(BaseModel):
        ...
        status: str = Field(..., regex="^(active|rollback|deprecated)$", description="Model status")
    ...
    class ModelsResponse(BaseModel):
        ...
        available_versions: List[ModelVersion] = Field(..., description="Available versions")
        deployment_history: List[ModelDeploymentHistory] = Field(..., description="Deployment history")
    ...
    class RollbackStep(BaseModel):
        ...
        status: str = Field(..., regex="^(pending|in_progress|completed|failed)$", description="Step status")
...
async def check_alert_conditions(metrics: MetricLogRequest):
    ...
    if metrics.inference.latency_ms > 100:
        ...
        logger.warning(f"High latency alert triggered: {metrics.inference.latency_ms}ms")
...
async def get_recent_alerts(
    ...
    if severity:
        filtered_alerts = [a for a in filtered_alerts if getattr(a, 'severity', None) == severity]
    
    if active_only:
        filtered_alerts = [a for a in filtered_alerts if getattr(a, 'status', None) == "active"]
    
    # Sort by timestamp (newest first) and limit
    filtered_alerts.sort(key=lambda x: getattr(x, 'timestamp', datetime.utcnow()), reverse=True)
    limited_alerts = filtered_alerts[:limit]
    
    if FASTAPI_AVAILABLE:
        return AlertsResponse(
            total_count=len(storage.alerts),
            active_count=len([a for a in storage.alerts if getattr(a, 'status', None) == "active"]),
...
async def acknowledge_alert(
    ...
    if not FASTAPI_AVAILABLE:
        return {"status": "acknowledged", "alert_id": getattr(request, 'alert_id', 'unknown')}
...
async def initiate_rollback(
    ...
    if not FASTAPI_AVAILABLE:
        return {"status": "initiated", "rollback_id": f"rollback_{uuid.uuid4().hex[:8]}"}
        
    # Validate target version exists
    if request.target_version not in storage.model_versions:
        raise HTTPException(status_code=404, detail=f"Model version {request.target_version} not found")
    
    # Generate rollback steps
    steps = [
        RollbackStep(step=1, description="Scale down current deployment", status="pending"),
