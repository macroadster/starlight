...
class DashboardGenerator:
    ...
    def update_dashboard(self, metrics: Dict[str, Any]) -> bool:
        ...
        content = content.replace(
            "**Model**: v4_production_quantized", "**Model**: v4_production_quantized"
        )  # keep as is
        content = content.replace(
        ...
        content = content.replace(
            "| False Positive Rate | 0.07% | <0.05% | ⚠️ Close |", fpr_line
        )
        ...
        for method in methods:
            ...
            line = (
                f"| {method.capitalize()} Detection | {rate:.1f}% | >99% | {status} |"
            )
        ...
        content = content.replace(
            "- **Latency p50**: 3.8ms", f'- **Latency p50**: {perf["p50_ms"]:.1f}ms'
        )
        content = content.replace(
            "- **Latency p95**: 4.2ms (Target: <5ms)",
            f'- **Latency p95**: {perf["p95_ms"]:.1f}ms (Target: <5ms)',
        )
        content = content.replace(
            "- **Throughput**: 238 img/sec",
            f'- **Throughput**: {perf["throughput_imgs_per_sec"]:.0f} img/sec',
        )
        ...
        content = content.replace(
            "- **Total Samples**: 5,200", f'- **Total Samples**: {dq["total"]}'
        )
        content = content.replace(
            "- **Method Balance**: LSB 22% | Alpha 19% | Palette 18% | EXIF 20% | EOI 21%",
            f"- **Method Balance**: {balance_str}",
        )
        content = content.replace(
            "- **Shannon Entropy**: 2.47 / 2.32 (excellent diversity)",
            f'- **Shannon Entropy**: {dq["entropy"]:.2f} / 2.32 (excellent diversity)',
        )
    ...
    def generate_html_dashboard(
        self, metrics: Dict[str, Any], output_path: str = "monitoring/dashboard.html"
    ) -> bool:
...
if __name__ == "__main__":
    ...
    sample_metrics = {
        ...
        "dataset_quality": {
            "balance": {"lsb": 22, "alpha": 19, "palette": 18, "exif": 20, "eoi": 21},
            "entropy": 2.47,
            "total": 5200,
        },
    }
